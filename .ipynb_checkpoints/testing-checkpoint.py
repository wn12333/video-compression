...
parser.add_argument("--output", type=str, default="/root/autodl-tmp/code/LHBDC/results/c-frames") 
    ...
    with torch.no_grad():
        ...
        for folder in folder_names:
            ...
            for idx, gop in enumerate(test_loader):
                ...
                if idx == 0:
                    dec0, size0 = i_compress(gop[0], i_model)
                ...
                if (idx + 1)%args.i_interval == 0:
                    ...
                    dec_last, size_last = i_compress(gop[-1], i_model)
                    frame_type = "I"
                    frame_num = 0
                else:
                    pass
                ...
                # print(infographic)
                # print("infographic数据类型", type(infographic))
                # print("infographic有多少数值", len(infographic))
                decoded = {0: dec_last}
                # print("重置decoded有多少数值", len(decoded))
...
def main(args):
    ...
    i_model = models[list(models.keys())[0]](quality=3, metric="mse", pretrained=True).to(device).float()   # compress coder
