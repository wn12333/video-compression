...
class ResDecodeNet(nn.Module):
    ...
    def __init__(self):
        ...
        self.deconv1 = nn.ConvTranspose2d(out_channel_resN, out_channel_resN, 5, stride=2, padding=2, output_padding=1)
        self.res1 = Resblocks()
...
class ResPriorEncodeNet(nn.Module):
    ...
    def __init__(self):
        ...
        self.conv1 = nn.Conv2d(out_channel_res, out_channel_res, 3, stride=1, padding=1)
        self.relu1 = nn.LeakyReLU(negative_slope=0.2)
        self.conv2 = nn.Conv2d(out_channel_res, out_channel_res, 5, stride=2, padding=2)
        self.relu2 = nn.LeakyReLU(negative_slope=0.2)
        self.conv3 = nn.Conv2d(out_channel_res, out_channel_res, 5, stride=2, padding=2)

    def forward(self, x):
        # print("先验编码器ResPriorEncodeNet", x.shape)
        x = self.relu1(self.conv1(x))
        # x = Padit(x)
        x = self.relu2(self.conv2(x))
        # print("先验编码器第三个x", x.shape)
        # x = Padit(x)
        return self.conv3(x)
...
class ResPriorDecodeNet(nn.Module):
    ...
    def __init__(self):
        ...
        self.deconv1 = nn.ConvTranspose2d(out_channel_res, out_channel_res, 5, stride=2, padding=2, output_padding=1)
        self.relu1 = nn.LeakyReLU(negative_slope=0.2)
        self.deconv2 = nn.ConvTranspose2d(out_channel_res, out_channel_res * 3 // 2, 5, stride=2, padding=2, output_padding=1)
        self.relu2 = nn.LeakyReLU(negative_slope=0.2)
        self.deconv3 = nn.ConvTranspose2d(out_channel_res * 3 // 2, out_channel_res * 2, 3, stride=1, padding=1)
