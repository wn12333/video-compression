...
class TestInfographic():
    ...
    def plot_psnr_vs_bpp(self, file_name):
        psnr_values = []
        bpp_values = []

        # 假设您已经有了一个名为frame_df的DataFrame，其中包含名为'psnr'和'pixel_values'的列
        psnr_values = self.frame_df['psnr']
        bpp_values = self.frame_df["size"] / self.frame_df["pixels"]

        sorted_indices = np.lexsort((bpp_values, psnr_values))

        sorted_bpp_values = bpp_values[sorted_indices]
        sorted_psnr_values = psnr_values[sorted_indices]    

        # 创建一个新的图形和坐标轴
        fig, ax = plt.subplots()

        # 将PSNR值作为y轴，像素值作为x轴，将它们绘制在图上
        ax.plot(sorted_bpp_values, sorted_psnr_values, marker='o')

        # 设置坐标轴标签
        ax.set_xlabel('BPP values')
