...
# prior the test
parser.add_argument("--project_name", type=str, default="LHBDC")  # Project name
parser.add_argument("--model_name", type=str, default="LHBDC")  # Model name

parser.add_argument("--test_path", type=str, default="datastes/UVG/")  # Dataset paths

parser.add_argument("--test_gop_size", type=int, default=5)  # Test gop sizes
parser.add_argument("--i_interval", type=int, default=5)  # Test i compression interval
...
parser.add_argument("--b_pretrained", type=str, default="results/final_model_1725082692.8887305.pth")  # Load model from this file
parser.add_argument("--output", type=str, default="/root/autodl-tmp/code/LHBDC/testing")
parser.add_argument("--i_qual", type=int, default=3)
...
# Frame order for decoding (coding order for a 5-frame video)
coding_order = [0, 4, 2, 1, 3]

# prev_frame, future_frame, frame_level (for each non-reference frame)
decoding_info = {
    2: [0, 4],  # Frame 2 depends on Frames 0 and 4
    1: [0, 2],  # Frame 1 depends on Frames 0 and 2
    3: [2, 4]   # Frame 3 depends on Frames 2 and 4
}

# Hierarchical levels for each frame
hier_levels = {
    0: 0,  # Keyframe, highest level
    4: 4,  # First level of dependent frames
    2: 2,  # Second level
    1: 3,  # Third level
    3: 3   # Third level
}
...
def i_compress(im, model):
    # out1 = model(im)
    out1 = model.invcompress.forward(im)
    dec1 = out1["x_hat"]
    size_image = sum(
...
def test(b_model, device, args):
    ...
    with torch.no_grad():
        ...
        for folder in folder_names:
            ...
            # Loading videos in batches of form I-B-B-B-I
            for idx, gop in enumerate(test_loader):
                # print("idx,gop", idx, gop.shape)

                gop = gop.unsqueeze(1).to(device)

                if idx == 0:
                    dec0, size0 = i_compress(gop[0], b_model)
                ...
                if (idx + 1) % args.i_interval == 0:
                    ...
                    dec_last, size_last = i_compress(gop[-1], b_model)
                    frame_type = "I"
                    frame_num = 4
                ...
                for order in coding_order_eff:
                    ...
                    infographic.update(
                        "B", frame_num, args.i_qual, folder, cur_psnr, dec_size.item(), float(h * w))
...
def main(args):
    ...
    b_model.load_state_dict(state_dict)
    b_model = b_model.to(device).float()

    logging.info("final_model_path: %s" % final_model_path)
    start_time = time.time()
    logging.info("start_testing_time: %s" % start_time)
    ...
    if args.log_results:
        ...
        # infographic.plot_psnr_vs_bpp(f"plot_psnr_vs_bpp.png")
