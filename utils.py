...
class TestInfographic():
    ...
    def print_per_video_level(self, video):
        ...
        for (vid_level, sums), (_, num_frames) in zip(self.totals_per_video_level.iterrows(),
            ...
            logging.info(
                "Video: " + str(video) + ", Level: " + str(level) + " bpp: " + str(sums["size"] / sums["pixels"]))
    # def print_per_video_level(self, video):
    #     # print("self.frame_df里的内容", self.frame_df)
    #     # print(self.frame_df.to_string(index=False, header=True, justify='center', width=10))
    #
    #     grouped = self.frame_df.groupby(["video", "level"])
    #
    #     self.totals_per_video_level = grouped.sum().xs(video, level=0, drop_level=False)
